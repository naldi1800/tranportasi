...
public class GameController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (puzzle && Time.timeScale > 0 && !gameFinished)
        {
            ...
            ProcessTimer();
            if (elapsedTimeUI)
                elapsedTimeUI.text = GetElapsedTime();
        }
        ...
        if (piecesLeftUI)
            piecesLeftUI.text =
                puzzle.remainingPieces.ToString() + " / " + puzzle.pieces.Length.ToString();
    }
    ...
    string SecondsToTimeString(float _seconds)
    {
        ...
        return hours_tmp.ToString()
            + ":"
            + minutes_tmp.ToString()
            + ":"
            + seconds_tmp.ToString("00");
    }
    ...
    static Vector3 GetPointerPosition(Camera _camera)
    {
        ...
        if (Input.touchCount > 0)
            pointerPosition = oldPointerPosition = _camera.ScreenToWorldPoint(
    ...
    public void AlignCameraWithBackground(float _sizeTuner = 0)
    {
        ...
        if (gameCamera.aspect > 1)
            gameCamera.orthographicSize =
                background.bounds.size.x / (gameCamera.aspect * 2) + _sizeTuner;
        else
    ...
    {
        if (
            background /*&&  background.transform.parent != puzzle.transform*/
        )
        {
            ...
            if (background as SpriteRenderer)
            {
                ...
                background.transform.localScale = new Vector3(
                    puzzle.puzzleBounds.size.x / background.bounds.size.x,
                    puzzle.puzzleBounds.size.y / background.bounds.size.y,
                ...
                if (
                    Mathf.Abs(background.bounds.min.x - puzzle.puzzleBounds.min.x) > 1
    ...
    {
        if (gameFinished || remainingHints == 0)
            return;
        else
