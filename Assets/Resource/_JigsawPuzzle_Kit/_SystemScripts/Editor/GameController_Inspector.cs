...
public class GameController_Inspector : Editor 
{
	...
	{   
		...
		if(showGameUI)
		{
			...
			if (game.timer >= 0) game.elapsedTimeUI = EditorGUILayout.ObjectField(new GUIContent("Elapsed time", "Link to UI text to visualize elapsed  time (if timer enabled)"), game.elapsedTimeUI, typeof(Text), true) as Text;
			game.piecesLeftUI = EditorGUILayout.ObjectField(new GUIContent("Pieces Left", "Link to UI text to visualize pieces left unassembled"), game.piecesLeftUI, typeof(Text), true) as Text;
